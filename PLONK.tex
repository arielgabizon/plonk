...
\author{ Ariel Gabizon\thanks{Most of this work was done while the first author was working at Protocol Labs.}\\ \tt{Aztec}  \and Zachary J. Williamson\\ \tt{Aztec} \and Oana Ciobotaru\\ \tt{Pi Squared}} 
...
\newcommand{\spartan}{\ensuremath{\mathsf{Spartan}}}
\newcommand{\spark}{\ensuremath{\mathsf{SPARK}}\xspace}
...
A significant technical contribution of the recent\footnote{In hindsight, we realized \spartan\cite{spartan} gave a similar sparse evaluation scheme for multilinear polynomials prior to \cite{marlin,fractal}.}  concurrent \fractal and \marlin systems \cite{marlin,fractal}
...
We claim that the reduction of Lemma \ref{lem:ideal-to-AGM} can be changed so that only $\reducedelems$ $\F$-elements are sent by \prv.
\begin{enumerate}
...
\paragraph{Enforcing constants:}
It is quite convenient and direct to set constant values: Fix any $j\in [m], a\in \F$.  To enforce the constraint $\assignment_j = a$ we set  for some $i\in [n]$
\[\lvar_i=j,    (\selleft)_i=1 ,(\selmult)_i= (\selright)_i=(\selout)_i = 0 , (\selconst)_i =-a.\]  
...
The following claim is crucial for the correctness of the protocols in Section \ref{sec:permprotocol}.
\begin{claim}\label{clm:randpermO}
Fix any permutation $\sigma$ of $[n]$, and any $a_1,\ldots,a_n,b_1,\ldots,b_n \in \F$.
 If 
 \[\prod_{i\in [n]} (a_i + \beta \cdot i + \gamma) = \prod_{i\in [n]} (b_i+ \beta \cdot \sigma(i) + \gamma)\] 
 with probability larger than $n/|\F|$ over uniform $\beta,\gamma \in \F$;
 then  $b_i = a_{\sigma(i)}$ for all $i \in [n]$.
\end{claim}

% We will prove claim \ref{clm:randpermO} using the well-known Schwartz-Zippel lemma.
% 
% \begin{lemma}(Schwartz-Zippel)\label{le:SchwartzZippel}. Let $P(X_1, \ldots, X_m)$ be a non-zero multivariate polynomial of degree $n$ over $\F$, then
% the probability that  $P(\alpha_1, \ldots, \alpha_m)= 0 $ for uniformly chosen $\alpha_1, \ldots, \alpha_m$ is at most $n/|\F|$.
% \end{lemma}

% The Schwartz-Zippel lemma is used in polynomial equality testing. Given two multi-variate polynomials $P_1(X_1, \ldots, X_m)$
% and $P_2(X_1, \ldots, X_m)$ we can test whether $P_1(\alpha_1, \ldots, \alpha_m) -  P_2(\alpha_1, \ldots, \alpha_m)  = 0$ for random $\alpha_1, \ldots, \alpha_m  \leftarrow \mathbb \F$.
% If the two polynomials are identical, this will always be true, whereas if the two polynomials are different then the equality holds with probability at most $max(n_1, n_2)/|\F|$, where $n_1$ and 
% $n_2$ are the degrees of the polynomials $P_1$ and $P_2$. 
...
By Schwartz-Zippel and the assumption of the claim, the following equality of polynomials holds in $\F[X,Y]$.
\[\prod_{i=1}^n(a_i+iX +Y) \equiv  \prod_{i=1}^n(b_i+\sigma(i)X +Y).\]
 Recall that $\F[X,Y]$ is a unique factorization domain where the invertible elements are exactly $\F^*$. Note also that the linear factors in the above products are irreducible. Thus, there must be a one to one map $M\to M'$ between the factors of each side, such that $M=c\cdot M'$ for some $c\in \F^*$.
% As $\F[X,Y]$ is a unique factorization domain and all coefficients of $Y$ appearing in the above decomposition into irreducible elements are $1$, comparing the coefficients of $X$, we must have
Moreover, since the coefficient of $Y$ in all terms on both sides is one, we must always have $c=1$.
Thus, this mapping must map a factor on the LHS to one on the RHS with the same coefficient for $X$.
In summary, we have for all $i\in [n]$, 
