...
\begin{itemize}
 ...
 $t, \sett{\cm_i,z_i,s_i=f_i(z)}{i\in [t]}$, \verpc outputs \acc with probability one.
...
\end{dfn}
\begin{remark}
We intentionally do not define a zero-knowlege property for idealized protocols, as acheiving ZK will depend on how much information on the polynomials sent to \ideal is leaked in the final ``compiled'' protocol. This in turn depends on specific details of the polynomial commitment scheme used for compilation.
\end{remark}
...
\end{array}$$
	...
	+ \left( (\bar{a} + \beta \chalpoint + \gamma)(\bar{b} + \beta k_1 \chalpoint + \gamma)(\bar{c} + \beta k_2 \chalpoint + \gamma) \cdot \idpermpoly(X) \right)\alpha \\
	- \left( (\bar{a} + \beta \sigpolyevala + \gamma)(\bar{b} + \beta \sigpolyevalb + \gamma)\beta \bar{z}_\omega \cdot \sigpoly_3(X)\right)\alpha \\
	+ \left( \idpermpoly(X) \right) \lagrangepoly_1(\chalpoint)\alpha^2 \\
	\end{array}
	$$
 ...
 \selmultcomm := \selmultpoly(x) \cdot [1]_1, \selleftcomm := \selleftpoly(x) \cdot [1]_1, \selrightcomm := \selrightpoly(x) \cdot [1]_1, \seloutcomm := \seloutpoly(x) \cdot [1]_1, \\
	...
	x \cdot [1]_2, c:=P(\omega),
	\end{array}
	$$
	where $P(X):=\frac{X^n-1}{X-\omega}$.
...
\item	Compute Lagrange polynomial evaluation $\lagrangepoly_1(\chalpoint) = \frac{\chalpoint^n - 1}{c(\chalpoint - \omega)}$.
